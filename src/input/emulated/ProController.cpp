...
bool ProController::set_default_mapping(const std::shared_ptr<ControllerBase>& controller)
{
	...
	case InputAPI::WGIGamepad:
		// The UWP host publishes WGI buttons in SDL-style slots (A=0,
		// B=1, X=2, Y=3). Match the VPAD WGI mapping so pre-created
		// multiplayer Pro Controllers receive a complete default profile.
		mapping =
		{
